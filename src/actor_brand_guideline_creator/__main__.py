...
"""
    ...
    prompt = f"""You are analyzing design system files for {domain} and generating an llms.txt file.

**Files from KV store ({len(files_content)} total)**:
{files_list}

**Expected Output Format** (based on ./example/brand-guideline.md reference):
- Start with: # {{Brand}} Brand Styling
- Include Overview section with brief description
- Include Keywords for searchability (e.g., "branding, corporate identity, visual identity...")
- Brand Guidelines section with Colors (hex codes) and Typography (font families, weights)
- Features section describing key design elements and patterns
- Technical Details if relevant

**Your Task**:
Analyze the provided files and generate a comprehensive llms.txt that follows the example format. Extract actual values:
1. Brand colors (hex codes, RGB values)
2. Typography (font families, weights, sizes)
3. Design tokens and CSS variables
4. Component patterns
5. Usage guidelines
6. Technical implementation details

Output ONLY the llms.txt markdown content. Be specific and use actual extracted values from the files. Write the complete file to llms.txt."""

    try:
        # Configure Claude SDK options (simple mode without MCP servers for text generation)
        options = ClaudeAgentOptions(
            model="sonnet",
        ...
        async with ClaudeSDKClient(options=options) as client:
            ...
            # Collect all responses
            async for message in client.receive_response():
                Actor.log.info(f"Claude: {message}")
                responses.append(str(message))

        # Extract the generated content from responses
